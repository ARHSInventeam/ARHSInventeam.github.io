...
class SartopoSession():
    ...
    def sendRequest(self,type,apiUrlEnd,j,id="",returnJson=None,timeout=None,domainAndPort=None):
        ...
        if internet:
            ...
            if not self.key or not self.id:
                ...
                return False
        #if apiUrlEnd == "Livetrack":
            #apiUrlEnd = "LiveTrack"
        #apiUrlEnd.replace("Livetrack", "LiveTrack")
            
        #if "Livetrack" in apiUrlEnd:
            #logging.info("ILEKHBNA:OLKLM<EDKSYIGKH>MLE:OUDLISFAYHGLBNEO:APLS")
            #apiUrlEnd.replace("Livetrack", "LiveTrack")
        #apiUrlEnd = "LiveTrack/b65873b9-b8a9-45a3-891e-40af0a401037" 
        
        print(f"prefix: {prefix}, dap: {domainAndPort}, mid: {mid}, apiurlend: {apiUrlEnd}")
        if "Livetrack" in apiUrlEnd:
            apiUrlEnd = apiUrlEnd.replace("Livetrack", "LiveTrack")
        ...
        logging.info(f"apiurled:   {apiUrlEnd}")
        ...
        if type=="post":
            ...
            if internet:
                ...
                logging.info(f"given ID: {self.id}")
            ...
            #r=requests.get(url,data=params,timeout=timeout,proxies=self.proxyDict,allow_redirects=False)
            #r=requests.post(url,data=params,timeout=timeout,allow_redirects=False)
            #print(r.json())
            ...
            if internet:
                ...
                r= requests.get(url,params=params,timeout=timeout,proxies=self.proxyDict,allow_redirects=False)
                logging.info(f'internet :  {r}')
                logging.info(f'internet result:     {r.json()}')
            else:
                #r=self.s.get(url,timeout=timeout,proxies=self.proxyDict)
                r=requests.get(url,timeout=timeout,proxies=self.proxyDict)
                logging.info(f'not internet :  {r}')
    ...
    def editFeature(self,
        ...
        if properties is not None:
            ...
            #logg   ing.info('proptitle:  ' + str(propToWrite['title']))
        ...
        logging.info('ID:  ' + str(feature["id"]))
        logging.info('classname:  ' + str(className))
...
#checkpoint
        ...
        #return self.sendRequest('post',className,j,id=feature['id'],timeout=timeout)
        #logging.info('return json:  ' + str(testVar))
        #return self.sendRequest('post',className,j,id=feature['id'],returnJson="ID",timeout=timeout)
        return self.sendRequest('post',className,j,id=feature['id'],returnJson="ID",timeout=timeout)
